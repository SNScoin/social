...
class InstagramParserEnhancedTest:
    """Test version of the enhanced Instagram parser that uses existing HTML"""
    ...
    async def parse_url(self, url: str) -> dict:
        """Parse Instagram URL using existing HTML response"""
        if not self.validate_url(url):
            raise ValueError("Invalid Instagram URL format")
        
        try:
            # Use existing HTML response instead of ScrapeNinja API
...
async def test_enhanced_instagram_with_html():
    """Test the enhanced Instagram parser using existing HTML"""
    url = "https://www.instagram.com/reel/DLZEet2I6No/?utm_source=ig_web_copy_link"
    
    print("🚀 Testing Enhanced Instagram Parser with HTML")
    print("=" * 60)
    print(f"URL: {url}")
    print()
    ...
    try:
        # Initialize parser
        parser = InstagramParserEnhancedTest()
...
if __name__ == "__main__":
    asyncio.run(test_enhanced_instagram_with_html()) 